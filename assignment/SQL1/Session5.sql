CREATE TABLE tblAttendance (
  ...
  AttendanceMonth VARCHAR(255) NOT NULL,
  NumberAttendance INT NOT NULL,
  FOREIGN KEY (EmployeeNumber) REFERENCES tblEmployee(EmployeeNumber)  -- This enforces referential integrity
);
...
DECLARE @go as geometry
select @go = GXY from tblGeom where IDtblGeom = 5
...
DECLARE @ga as geography
select @ga = GXY from tblGeog where IDtblGeog = 1
...
DECLARE @ho as geography

select @g = GXY from tblGeog where IDtblGeog = 1
select @ho = GXY from tblGeog where IDtblGeog = 2
select @g.STDistance(@h) as MyDistance
